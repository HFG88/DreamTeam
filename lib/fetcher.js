...
export async function fetchJson(file) {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Gat ekki sótt ${file}: ${response.status} ${response.statusText}`
      );
    }
...
export async function fetchLectures(category) {
  return await fetchJson(`./public/data/${category}/lectures.json`);
}
